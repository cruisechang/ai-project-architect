...
- 通过 `apa prompt` 输出结构化提示词，让 AI agent 在可控流程里持续推进
...
# 交互模式
./apa init
# 会直接进入 Wizard，第一题是 Project idea。
...
`apa prompt` 会读取当前 repo 状态，并输出一段给 AI agent 的结构化提示词。

它可以在实现前、开发中，或回归修复后使用，帮助 agent 按照现有文档、任务和约束持续推进到交付。

它也会检查现有文档是否采用按优先级对齐的 `Phase 0`、`Phase 1`... 段落；如果不是，`apa prompt` 会提醒用户，并要求 agent 先用 `apa-docs` 重写文档，再继续实现。
...
`apa-doc-review` 用于文档优先的交互修订：每轮只改文档、改完就停下等待反馈，在你明确说 `docs approved` 之前不得开始实现。
